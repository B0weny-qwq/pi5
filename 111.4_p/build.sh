...
# 当前文件夹是第4问速度模式串级控制版：钢球PI-D输出目标水管角度，
...
g++ -O2 "${common_flags[@]}" task4_balance_control_test.cpp \
    -o task4_balance_control_test "${opencv_flags[@]}"
./task4_balance_control_test

# 测试通过后才生成比赛正式程序，避免误运行旧二进制文件。
g++ -O3 -DNDEBUG "${common_flags[@]}" main.cpp \
    -o ball2_task4_velocity "${opencv_flags[@]}"

echo "Build complete: ./ball2_task4_velocity"
