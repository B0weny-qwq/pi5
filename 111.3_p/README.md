...
每次启动都会覆盖写入当前目录的 `ball2_task3_runtime.log`，并同步打印到终端。启动行
记录串口、细分、ZDT `0x30/0x35/0x3A/0x3B` 状态；随后每 `200 ms` 记录一次钢球位置、
速度、是否已 ARM、O 点稳定帧数、目标/实际电机脉冲、实际 RPM、控制 RPM 和实际下发
的 F6 RPM。按空格被拦截、ARM、丢球、串口失败和退出也会写入事件行。
