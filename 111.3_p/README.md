...
钢球运动状态机仍采用经过实机验证的分阶段思路：

```text
O -> O+5：固定0.35°驱动，在+3.0 cm边界提前折返并利用惯性到达+5
O+5 -> O-5：以-4.5 cm/s为目标闭合钢球速度环
距O-5约0.8 cm：切换位置+速度控制，允许最大0.10192°制动
O-5附近：精确以x=176为目标，完成范围收紧为左0.20/右0.02 cm
```

这次只重写执行器层，不同时推翻钢球阶段逻辑。先把电机对目标水管角度的跟随变得
可测、可限速、可控加速度，再根据新实机视频调整钢球参数，能避免两个环同时变化。
...
3. 水管角度正确但钢球轨迹不对：再调第3题角度、制动距离和终点参数。
